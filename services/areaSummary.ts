...
const getSummaryOfCategory = (
  ...
  return {
    ...
    tops: business.slice(0, 5).map(b => ({
      ...
      parent_categories: _.flattenDeep(
        b.categories.map(c => c.parent_categories.map(pc => pc.title))
      )
    }))
  };
};
...
export const storeAreaSummaries = (area: string): Promise<IArea> => {
  ...
  const promises = PLACE_INTEREST.map((category, index) =>
    getFactory(category).then(result => {
      if (result.errors) {
        logger.error({
  ...
  return Promise.all(promises)
    .then(() => {
      ...
      return AreaModel.findOneAndUpdate(
        ...
        throw err;
      }); // overloads: should return promise
    })
    .then(() => areaDoc);
};
...
export const getAreaSummaries = (area: string): Promise<IArea> => {
  return AreaModel.findOne({
    ...
    .exec() // !findOne().then() is not a full-fludged promise
    .then(res => {
...
export const getPhotosFromArea = (areaSummary: IArea): IPhoto[] => {
  const places = _.flatten(
    areaSummary.place_of_interest_summaries.map(summary => summary.tops)
  );
...
export const getAllPhotos = (area: string): Promise<IPhoto[]> => {
  return AreaModel.findOne({ term: area })
    .exec()
    .then(area => {
      if (!area) {
    ...
    .catch(err => {
      if (!(err instanceof NonNullableGraphQLError)) {
        throw err;
      }
      // TODO unsure, test
